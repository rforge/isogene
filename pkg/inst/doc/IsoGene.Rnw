...
<<IsoRawp>>=
  rawp <- IsoRawp(x = x, y = exampleData, niter = 2) # TODO change to 1000 in final version
@
...
#pdf (file = "IsoGene-IsoSAMPlotOutput.pdf", width = 7, height = 7)
png (filename = "IsoGene-IsoSAMPlotOutput.pdf", width = 480, height = 480)
...
\begin{figure}[!h]
\centering
{\includegraphics[width=0.5\textwidth]{IsoGene-IsoSAMPlotOutput.png}}
