...
\author{\large \bf Dan Lin$^1$, Setia Pramana$^1$, Ziv Shkedy$^1$and Tobias Verbeke$^2$  \\[2cm]\\
...
<<IsoPlot, fig = F, include = FALSE,eval=F >>=
...
<< Plot, fig = F, include = FALSE, echo=false,cache=true>>=
...
<<IsopvaluePlot, fig=TRUE, include=FALSE, eval=FALSE>>=
  IsopvaluePlot(x, gene1, niter=1000, stat="E2")
@


<< Plot3, fig = F, include = FALSE, echo=false,cache=true>>=
...
<<IsoBHPlot,fig=F, include=FALSE, eval=F>>=
	IsoBHPlot(rawp.twosided, FDR=0.05, stat="E2")
@


<< Plot2, fig = F, include = FALSE, echo=false,cache=true>>=
...
<<IsoSAMPlotOutput, fig=FALSE, include = FALSE, eval=F>>=
	IsoSAMPlot(qqstat = qqstat, allfdr = dtable, FDR = 0.05, stat = "ModifM")
@

<< Plot4, fig = F, include = FALSE,echo=false,cache=true>>=
pdf (file = "IsoGene-IsoSAMPlotOutput.pdf", width = 7, height = )
IsoSAMPlot(qqstat = qqstat, allfdr = dtable, FDR = 0.05, stat = "ModifM")
graphics.off() 
@
...
\begin{figure}[!h]
\raggedright
{\includegraphics[width=1.3\textwidth]{IsoGene-IsoSAMPlotOutput.pdf}}
\caption{\em {The SAM plots: a.Plot of the FDR vs. delta; b. Plot of number of significant 
